...
export const useProductStore = create(set => ({
	...
	fetchProductById: async productId => {
		...
		setLoading(true);
		try {
			const res = await api.get(`${import.meta.env.VITE_API_URL}/products/${productId}`, {
				timeout: 15000
			});
			return res.data || null;
		} catch (err) {
	...
	fetchProductsByBaker: async bakerId => {
		...
		set({ error: '' });
		try {
			const res = await api.get(`${import.meta.env.VITE_API_URL}/products/bakers/${bakerId}`, {
				timeout: 15000
			});
			set({ bakerProducts: res.data || [] });
		} catch (err) {
	...
	deleteProduct: async (productId, token) => {
		...
		try {
			const res = await api.delete(`${import.meta.env.VITE_API_URL}/products/${productId}`, {
				headers: { Authorization: `Bearer ${token}` },
				timeout: 15000
			});
	...
	updateProduct: async (productId, productData, token) => {
		...
		try {
			const res = await api.put(`${import.meta.env.VITE_API_URL}/products/${productId}`, productData, {
				headers: {
					Authorization: `Bearer ${token}`,
				},
				timeout: 30000,
